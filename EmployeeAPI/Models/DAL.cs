...
namespace EmployeeAPI.Models
{
    public class DAL
    {
        public Response GetAllEmployees(SqlConnection connection)
        {
            ...
            SqlDataAdapter dataAdapter = new SqlDataAdapter("SELECT * FROM Emplyees", connection);
        ...
        {
            Response response = new Response();
            SqlDataAdapter dataAdapter = new SqlDataAdapter("SELECT * FROM Emplyees WHERE ID = '"+id+"' AND IsActive = 1", connection);
            DataTable dataTable = new DataTable();
            Employee employee = new Employee();
